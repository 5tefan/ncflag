...
class TestTheoretical(TestCase):
    ...
    def test_exclusive_flag_type(self):
        ...
        np.testing.assert_array_equal(f.get_flag("good"), np.array([1, 1, 0, 0, 0, 0]),
                                      err_msg="only the first two are good")
        np.testing.assert_array_equal(f.get_flag("medium"), np.array([0, 0, 1, 0, 0, 0]),
                                      err_msg="only 3d element is medium")
        np.testing.assert_array_equal(f.get_flag("bad"), np.array([0, 0, 0, 1, 0, 0]),
                                      err_msg="only the 4th element is bad")
        np.testing.assert_array_equal(f.get_flag("extra_bad"), np.array([0, 0, 0, 0, 1, 0]),
                                      err_msg="only second to last is extra_bad")
        np.testing.assert_array_equal(f.get_flag(["good", "bad"]), np.array([1, 1, 0, 1, 0, 0]),
                                      err_msg="good and bad weren't merged together.")

        self.assertTrue(len(f.get_flags_set_at_index(-1)) == 0, msg="No flags set at the end. Fill value.")
        ...
        for i in range(len(f.flags)):
            ...
            self.assertTrue(
                f.get_flags_set_at_index(i, exit_on_good=True) == f.get_flags_set_at_index(i, exit_on_good=False)
            )
        ...
        np.testing.assert_array_equal(f.get_flag("extra_bad"), np.array([1, 0, 0, 0, 1, 0]))
        ...
        # assert still in original form
        np.testing.assert_array_equal(f.flags, original_flags, err_msg="Flags not back to original!")

        # continuing now to test the full vector set_flag
        f.set_flag("medium", [1, 0, 0, 0, 1, 0], zero_if_unset=False)
        np.testing.assert_array_equal(f.get_flag("medium"), np.array([1, 0, 1, 0, 1, 0]))
        ...
        np.testing.assert_array_equal(f.get_flag("extra_bad"), np.array([1, 1, 0, 0, 0, 0]))
        ...
        for flag_meaning in f._flag_meanings:
            np.testing.assert_array_equal(f.find_flag(these_dont_exist + [flag_meaning]), f.get_flag(flag_meaning))
    ...
    def test_maskedarray_initial_flags(self):
        ...
        for flag_meaning in f._flag_meanings:
            ...
            f.set_flag(flag_meaning, np.zeros_like(original_flags), zero_if_unset=False)  # identity test
            np.testing.assert_array_equal(f.get_flag(flag_meaning), np.zeros(5))
        ...
        np.testing.assert_array_equal(f.flags.mask, [1, 0, 1, 0, 1])  # make sure it's still masked
        np.testing.assert_array_equal(np.ma.filled(f.flags, fill_value=9),
    ...
    def test_inclusive_flag(self):
        ...
        np.testing.assert_array_equal(f.get_flag("degraded"), np.array([0, 0, 1, 0, 1, 0]))
        np.testing.assert_array_equal(f.get_flag("middle"), np.array([1, 0, 0, 0, 0, 0]))
        ...
        np.testing.assert_array_equal(f.flags, original_flags | np.array([1, 0, 0, 0, 0, 0]))
        ...
        np.testing.assert_array_equal(f.get_flag("blue"), np.array([0, 0, 0, 1, 0, 1]))  # <- covered by original_flags
        ...
        np.testing.assert_array_equal(f.flags, original_flags)  # and now we're back to the original "blue" flags,
        ...
        # just reassigning.
        exclusive_set_original_setting = f.flags & (2 | 4)  # -- this is the mask that defines this exclusive set
        ...
        for flag_meaning in exclusive_set:
            ...
            f.set_flag(flag_meaning, np.ones_like(original_flags), zero_if_unset=False)  # set them all...
            # test that none of the others appear
            for should_not_be_set in exclusive_set - {flag_meaning}:
                np.testing.assert_array_equal(f.get_flag(should_not_be_set), np.zeros_like(original_flags))
